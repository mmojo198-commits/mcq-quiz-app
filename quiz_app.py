...
if st.session_state.questions is not None and not st.session_state.quiz_started:
    ...
    with st.container(border=True):
        ...
        if st.button("🚀 Start Quiz", use_container_width=True, type="primary"):
            ...
            st.session_state.time_spent = {}
            st.session_state.submitted_q = {}
            st.session_state.hint_visible = {} # Reset hints
...
# Calculate Time Remaining (Before UI)
remaining = None
...
# ==========================================
# ⚡ CRITICAL FIX: TIMEOUT CHECK (Before UI)
# ==========================================
if time_allowed is not None and not is_submitted and remaining is not None and remaining <= 0:
    st.session_state.time_spent[i] = time_allowed
    st.session_state.submitted_q[i] = True
    st.session_state.show_feedback_for = i
    
    # Try to salvage whatever the user selected before timeout
    ...
    st.rerun()     # Rerun immediately to show Feedback UI
...
with st.sidebar:
    ...
    st.divider()
    if st.button("🏁 Finish Quiz", use_container_width=True, type="secondary", key="finish_sidebar"):
        save_time_state()
...
with st.container(border=True):
    ...
    # Restore selection from saved answers or default to None
...
if not is_submitted:
    if pd.notna(row.get("Hint")):
        # Use explicit session state + callback for robustness
        hint_key = f"hint_state_{i}"
        is_checked = st.session_state.hint_visible.get(hint_key, False)
        
        if st.checkbox("Show Hint", value=is_checked, key=f"chk_hint_{i}", on_change=toggle_hint):
...
with col_prev:
    if st.button("⬅️ Previous", disabled=(i == 0), use_container_width=True, key="btn_prev"):
        handle_navigation(i - 1)

with col_submit:
    if not is_submitted:
        if st.button("🔒 Submit Answer", type="primary", use_container_width=True, key="btn_submit"):
            save_time_state()
...
with col_next:
    if i < total_q - 1:
        ...
        if st.button("🏁 Finish Quiz", type="primary", use_container_width=True, key="btn_finish"):
            save_time_state()
            curr = st.session_state.get(f"selected_option_{i}")
            if not is_submitted:
                st.session_state.answers[i] = curr
